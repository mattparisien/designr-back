...
interface PresetWithId extends ProjectPreset {
  id: string;
  key: string;
}

interface ProjectPresetsConfig {
  [category: string]: {
    [key: string]: ProjectPreset;
  };
}

const PROJECT_PRESETS: ProjectPresetsConfig = {
...
// Helper functions
const getPresetsByCategory = (category: string): { [key: string]: ProjectPreset } => {
  return PROJECT_PRESETS[category] || {};
};

const getPresetByKey = (category: string, key: string): ProjectPreset | null => {
  return PROJECT_PRESETS[category]?.[key] || null;
};

const getAllPresets = (): ProjectPresetsConfig => {
  return PROJECT_PRESETS;
};

const searchPresets = (searchTerm: string): PresetWithId[] => {
  const results: PresetWithId[] = [];
  Object.entries(PROJECT_PRESETS).forEach(([category, presets]: [string, { [key: string]: ProjectPreset }]) => {
    Object.entries(presets).forEach(([key, preset]: [string, ProjectPreset]) => {
      if (
...
const getPresetsList = (): PresetWithId[] => {
  const list: PresetWithId[] = [];
  Object.entries(PROJECT_PRESETS).forEach(([category, presets]: [string, { [key: string]: ProjectPreset }]) => {
    Object.entries(presets).forEach(([key, preset]: [string, ProjectPreset]) => {
      list.push({
        ...preset,
