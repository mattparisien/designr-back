 ...
 * Each local function is exposed to the LLM via `tool()` so it can decide when
 * to call them.  All tools return *strings* (JSON‑encoded when complex) to keep
 * the LLM context small and predictable.
 */
const searchAssetsTool = ({ vs }) =>
  tool({
    name: 'search_assets',
...
// Used for gathering external design inspiration or trend examples.
...
/**
 * Input guardrail -----------------------------------------------------------
 * Reject non‑design requests before they reach the model.
 */
const FORBIDDEN = [
  'politics',
  'election',
 ...
 * DesignAgentService --------------------------------------------------------
 */
export default class DesignAgentService {
  #agent;
  #initialized = false;
  ...
  async initialize() {
    ...
    this.#agent = new Agent({
      ...
      inputGuardrails: [designOnlyGuardrail],
    });
